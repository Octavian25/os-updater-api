...
module.exports = async function (fastify) {
  ...
  fastify.post("/enroll", async (request, reply) => {
    ...
    try {
      ...
      // Cek apakah data enroll sudah ada berdasarkan fingerprint
      ...
      if (existingEnroll) {
        return reply.code(200).send({
          message: "Enroll already exists",
          data: existingEnroll,
        });
      }

      // Simpan data enroll baru
      ...
      if (err.code === 11000) {
        return reply.code(200).send({ message: "Enroll already exists" });
      }
